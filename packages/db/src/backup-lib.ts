...
/**
 * Compatibility shape for callers that still pass the instance settings backup
 * retention object. Local disk pruning uses only `dailyDays`.
 */
...
type RunDatabaseBackupBaseOptions = {
...
export type RunDatabaseBackupOptions = RunDatabaseBackupBaseOptions & (
  | { localRetentionDays: number; retention?: BackupRetentionPolicy }
  | { localRetentionDays?: number; retention: BackupRetentionPolicy }
);
...
function normalizeLocalRetentionDays(value: number): number {
  const days = Math.trunc(value);
  return Number.isFinite(days) && days > 0 ? days : 1;
}

function resolveLocalRetentionDays(opts: RunDatabaseBackupOptions): number {
  return normalizeLocalRetentionDays(opts.localRetentionDays ?? opts.retention?.dailyDays ?? 7);
}

/**
 * Local backup pruning only applies to automatic timestamped backup files.
 * Manual/pre-rekey safety backups do not match that filename shape and are
 * preserved. Remote archive retention is managed outside this local disk prune.
 */
function pruneOldBackups(backupDir: string, localRetentionDays: number, filenamePrefix: string): number {
  if (!existsSync(backupDir)) return 0;

  const now = Date.now();
  const localCutoff = now - normalizeLocalRetentionDays(localRetentionDays) * 24 * 60 * 60 * 1000;
  ...
  for (const name of readdirSync(backupDir)) {
    ...
    if (stat.isFile() && stat.mtimeMs < localCutoff) {
...
export async function runDatabaseBackup(opts: RunDatabaseBackupOptions): Promise<RunDatabaseBackupResult> {
  ...
  const localRetentionDays = resolveLocalRetentionDays(opts);
