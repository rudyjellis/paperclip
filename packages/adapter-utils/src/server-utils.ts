...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Authorized managers or merge owners may close a direct report's plain `in_review` issue with PATCH /api/issues/{issueId} and a review comment (`status: done` to approve, `status: in_progress` to request changes) only when no typed execution participant, pending approval, or pending interaction already owns the review.",
