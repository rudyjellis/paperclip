...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Authorized managers or merge owners may close a direct report's plain `in_review` issue with PATCH /api/issues/{issueId} and a review comment (`status: done` to approve, `status: in_progress` to request changes) when no typed execution participant or pending approval owns the review. Pending interactions still block closeout unless they are legacy assignee-authored targetless `request_confirmation` review surrogates, which manager closeout now supersedes automatically with an audit trail.",
...
export function buildPaperclipEnv(
  ...
  options: { preferLocalListenUrl?: boolean } = {},
): Record<string, string> {
  ...
  const apiUrl =
    (options.preferLocalListenUrl && hasExplicitListenPort ? localApiUrl : null) ??
