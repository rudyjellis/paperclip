...
describeEmbeddedPostgres("issueThreadInteractionService", () => {
  ...
  it("expires pending request confirmations for authorized closeout comments", async () => {
    const { companyId, issueId } = await seedConfirmationIssue("Closeout confirmation supersede");
    ...
    const expired = await interactionsSvc.expireRequestConfirmationsSupersededByCloseoutComment({
      id: issueId,
      companyId,
    }, {
      id: commentId,
