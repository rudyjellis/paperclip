...
export type WorkspaceCleanupInventoryState = "eligible" | "blocked";
export type WorkspaceCleanupFinalizeState =
  | "verified"
  | "failed_latest"
  | "missing_proof"
  | "not_required"
...
export type WorkspaceGeneratedCleanupInventory = {
  state: WorkspaceCleanupInventoryState;
  reason: string | null;
  inspectionStatus: GeneratedCleanupWorktree["status"];
  liveProcessCheck: GeneratedCleanupWorktree["liveProcessCheck"];
...
export type WorkspaceRetirementInventory = {
  state: WorkspaceCleanupInventoryState;
  reasons: string[];
  hasRetireableAction: boolean;
  closeReadinessState: ExecutionWorkspaceCloseReadiness["state"];
  finalizeState: WorkspaceCleanupFinalizeState;
  finalizeReason: string | null;
  plannedActions: ExecutionWorkspaceCloseAction[];
  warnings: string[];
};

export type ExecutionWorkspaceCleanupInventoryItem = {
  workspaceId: string;
  ...
  nonTerminalLinkedIssueCount: number;
  hasNonTerminalLinkedIssues: boolean;
  linkedIssues: ExecutionWorkspaceCloseLinkedIssue[];
  git: ExecutionWorkspaceCloseReadiness["git"];
  generatedCleanup: WorkspaceGeneratedCleanupInventory;
  retirement: WorkspaceRetirementInventory;
};

export type ExecutionWorkspaceCleanupInventorySummary = {
  totalWorkspaces: number;
  generatedCleanupEligible: number;
  generatedCleanupBlocked: number;
  retirementEligible: number;
  retirementBlocked: number;
  reclaimableBytes: number;
  reclaimableCandidateCount: number;
};

export type ExecutionWorkspaceCleanupInventory = {
  generatedAt: string;
  companyId: string;
  minAgeHours: number;
  maxDepth: number;
  summary: ExecutionWorkspaceCleanupInventorySummary;
  workspaces: ExecutionWorkspaceCleanupInventoryItem[];
};
