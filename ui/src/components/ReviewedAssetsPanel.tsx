...
import type { ReviewedArtifactsResponse } from "@paperclipai/shared";
import {
  AlertTriangle,
  Copy,
  Download,
...
function toGalleryItem(artifact: ReviewedArtifactPanelItem): GalleryMediaItem {
  return {
    id: artifact.id,
    ...
    openPath: artifact.preview.previewUrl ?? "",
    downloadPath: artifact.preview.downloadUrl ?? undefined,
  };
}
...
function ReviewedArtifactEmptyState({ suggested }: { suggested: boolean }) {
  return (
    ...
    >
      {suggested ? "No suggested review assets are available yet." : "No reviewed assets selected yet."}
    </div>
  );
}
...
function ReviewedArtifactCard({
  ...
  const updatedAt =
    typeof artifact.snapshot?.updatedAt === "string" || artifact.snapshot?.updatedAt instanceof Date
...
export function ReviewedAssetsPanel({
  ...
  const galleryItems = useMemo(
    () => imageArtifacts.map((artifact) => toGalleryItem(artifact)),
    [imageArtifacts],
  );
  const panelTitle =
    title ??
    (model.hasSuggestedSelection && !model.hasExplicitSelection ? "Suggested review assets" : "Reviewed assets");
  const panelSubtitle =
    subtitle ??
    (model.hasSuggestedSelection && !model.hasExplicitSelection
