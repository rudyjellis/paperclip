...
vi.mock("./ImageGalleryModal", () => ({
  ImageGalleryModal: ({ items, open }: { items: unknown[]; open: boolean }) => {
    const firstImage = items[0] as { contentPath?: string } | undefined;
    return (
      <div
        data-testid="image-gallery"
        data-count={items.length}
        data-open={String(open)}
        data-content-path={firstImage?.contentPath ?? ""}
      />
    );
  },
}));
...
function response(
  ...
  errors: ReviewedArtifactsResponse["errors"] = [],
...
describe("ReviewedAssetsPanel", () => {
  ...
  it("renders markdown and image previews from backend preview mode", () => {
    ...
    expect(gallery?.getAttribute("data-content-path")).toBe("/image.png");
    act(() => root.unmount());
  });
  ...
  it("renders link and download-only assets with safe actions", () => {
    ...
    expect(container.querySelector('[data-testid="reviewed-artifact-download"]')?.textContent).toContain(
      "Download this asset",
    );
  ...
  it("renders unsupported, missing, and permission-denied asset states", () => {
    ...
    expect(container.querySelector('[data-testid="reviewed-artifact-unsupported"]')?.textContent).toContain(
      "SVG is download-only",
    );
    expect(container.querySelector('[data-testid="reviewed-artifact-missing"]')?.textContent).toContain(
      "Deleted after review",
    );
    expect(container.querySelector('[data-testid="reviewed-artifact-permission_denied"]')?.textContent).toContain(
      "Access denied",
    );
